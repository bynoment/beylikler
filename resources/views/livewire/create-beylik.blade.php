<div>
    <h2 style="margin:0 0 12px">Beylik OluÅŸtur</h2>
    ...
    <form wire:submit="create" style="display:grid;gap:10px; margin-bottom:16px">
        <div>
            <label>Ä°sim</label>
            <input type="text" wire:model.defer="name" placeholder="Ã–rn: Karesi" style="width:100%;padding:8px;border-radius:8px;border:1px solid #333;background:#0f1016;color:#f3f4f6" />
            @error('name') <div style="color:#fca5a5;font-size:12px">{{ $message }}</div> @enderror
        </div>
        <div class="muted">SeÃ§ilen il: <strong>{{ $selectedProvinceName ?: 'â€”' }}</strong></div>
        <div>
            <button type="submit" style="padding:10px 14px;border-radius:8px;background:#22c55e;color:#0b0b0f;border:0">OluÅŸtur</button>
        </div>
    </form>
